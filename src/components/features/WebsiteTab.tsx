...
import {
  ...
  ChevronRight, RotateCcw, X, Copy, Check,
  Package, Plus, Trash2, FolderOpen, Clock, ChevronLeft
} from 'lucide-react';
...
const PLANS = [
  {
    name: 'Free', price: '$0', period: '/month', coins: 500,
    coinLabel: '500 coins/day',
    features: ['DANI 1.15 only', 'Basic website generation', 'ZIP download', '10 coins/gen'],
    gradient: 'from-gray-400 to-gray-500', cta: 'Current Plan', disabled: true,
  },
  {
    name: 'Starter', price: '$4.99', period: '/month', coins: 2000,
  ...
  {
    name: 'Pro', price: '$14.99', period: '/month', coins: 10000,
  ...
  {
    name: 'Unlimited', price: '$29.99', period: '/month', coins: 99999,
...
function ProjectCard({
  project, isActive, onLoad, onDelete
}: {
  project: Project;
  isActive: boolean;
...
export default function WebsiteTab() {
  ...
  const saveProject = useCallback((name: string, desc: string, model: string, tech: string[], files: GeneratedFile[]): string => {
    ...
    const project: Project = {
      id, name, description: desc, model,
      techStack: tech, files,
  ...
  return (
    <div className="flex-1 flex overflow-hidden bg-gradient-to-br from-gray-900 via-purple-950 to-gray-900">
      ...
      {/* ── Daily Bonus Toast ── */}
      {dailyBonus > 0 && (
        <div className="fixed top-4 right-4 z-50 bg-gradient-to-r from-green-500 to-emerald-600 text-white px-5 py-3 rounded-2xl shadow-xl flex items-center gap-2 animate-[fadeIn_0.3s_ease]">
          <span className="text-xl">🎉</span>
          <div>
            <p className="font-bold text-sm">Daily coins refreshed!</p>
            <p className="text-xs text-green-100">+{dailyBonus} coins added to your balance</p>
          </div>
            ...
            {error && (
              <div className="mx-3 mb-3 p-3 bg-red-500/10 border border-red-500/30 rounded-xl flex items-start gap-2">
                ...
                <div>
                  ...
                  {error.includes('coin') && (
                    <button onClick={() => setShowPlans(true)} className="text-[11px] font-bold text-pink-400 hover:underline mt-1">
                      View Plans →
                    </button>
                  )}
                </div>
              ...
              <div className="p-3 border-t border-white/10">
                <button
                  onClick={() => {
                    ...
                    setProjects([]);
                    localStorage.removeItem('dani-website-projects');
                    setActiveProjectId(null);
                  }}
        ...
        <div className="flex items-center gap-3 px-4 py-3 border-b border-white/10 bg-black/20 flex-shrink-0">
          ...
          <div className="flex-1 flex items-center gap-2 min-w-0">
            ...
            {generatedFiles.length > 0 && activeProjectId && (
              <span className="text-[10px] px-2 py-0.5 rounded-full bg-pink-500/20 text-pink-400 border border-pink-500/20 flex-shrink-0">
                editing
              </span>
            )}
          </div>
            ...
            <div className="flex bg-white/5 rounded-xl p-0.5 gap-0.5 flex-shrink-0">
              {(['editor', 'preview'] as const).map(v => (
                <button key={v} onClick={() => setViewMode(v)}
                  className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-all flex items-center gap-1.5 ${viewMode === v ? 'bg-white/15 text-white' : 'text-gray-500 hover:text-gray-300'}`}>
          ...
          {generatedFiles.length > 0 && (
            <button onClick={handleDownload}
              className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-xl text-sm font-bold hover:shadow-lg hover:shadow-green-500/20 transition-all flex-shrink-0">
        ...
        {isGenerating ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center gap-8 px-8">
            ...
            <div className="flex flex-wrap justify-center gap-2">
              {['HTML + CSS + JS', 'React Apps', 'TypeScript', 'Live Preview', 'ZIP Export'].map(f => (
                <span key={f} className="px-3 py-1.5 rounded-full bg-white/5 border border-white/10 text-xs text-gray-500 font-medium">
                  ✦ {f}
                </span>
              ))}
            </div>
